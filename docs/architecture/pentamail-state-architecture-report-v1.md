...
Version 1.0.1 adds a read-only private reporting bridge for selected `integration_control` evidence. The bridge allows the report compiler to include PentaCertify task details, PentaGreen hourly state and PentaNurture cycle state without exposing the private schema through PostgREST. It also adds owner-visible PentaGreen execution-failure alerting.
...
The independent hourly `0 * * * *` clock subsequently generated and delivered another report without chat/manual invocation, proving that report delivery is not dependent on an interactive session.

After the private reporting bridge was added, a v1.0.1 corrected report successfully traversed the same production path and included detailed PentaCertify, PentaGreen and PentaNurture state. HIGH certification-plane and PentaGreen-execution incidents were also independently dispatched through the PentaMail priority outbox and received Mailgun HTTP 200 acknowledgements.
