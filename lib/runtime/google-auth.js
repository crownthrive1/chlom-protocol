...
async function createVercelFederatedClient() {
  const [googleAuthModule, vercelOidcModule] = await Promise.all([
    import('google-auth-library'),
    import('@vercel/oidc'),
  ]);
  const { ExternalAccountClient } = googleAuthModule;
  const { getVercelOidcToken } = vercelOidcModule;
...
async function createLocalAdcClient() {
