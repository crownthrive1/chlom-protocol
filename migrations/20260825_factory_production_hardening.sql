...
-- Keep the worker credential in Vault; only a digest comparison is exposed to the runtime.
do $$
begin
...
as $$
  select p_token is not null and exists(
    ...
    where name='ct_factory_worker_token'
...
begin
  ...
  values('factory.worker.dispatched','factory',jsonb_build_object('request_id',v_request_id,'limit',v_limit));
...
-- Replace the scheduler-only clock with the end-to-end dispatcher.
update cron.job set active=false where jobname='ct-software-factory-tick-v2';
select cron.schedule('ct-software-factory-dispatch-v3','* * * * *','select public.ct_factory_dispatch_tick();')
where not exists(select 1 from cron.job where jobname='ct-software-factory-dispatch-v3');

update public.ct_factory_projects
set repo_full_name='crownthrive1/chlom-protocol',
