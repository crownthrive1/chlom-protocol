...
pub mod pallet {
    ...
    pub type IdentityVersions<T: Config> = StorageDoubleMap<
        ...
        Blake2_128Concat,
        u32,
        IdentityVersion<BlockNumberFor<T>>,
        OptionQuery,
    >;
    ...
    pub type CredentialVersions<T: Config> = StorageDoubleMap<
        ...
        Blake2_128Concat,
        u32,
        CredentialVersion<BlockNumberFor<T>>,
        OptionQuery,
    >;
    ...
    impl<T: Config> Pallet<T> {
        #[pallet::call_index(0)]
        #[pallet::weight(T::WeightInfo::record_identity_version())]
        pub fn record_identity_version(
            ...
            T::AdminOrigin::ensure_origin(origin)?;
                ...
                subject_id != ZERO_ID
                    && did_hash != ZERO_ID
                    && fingerprint_hash != ZERO_ID
                    && controller_subject_id != ZERO_ID
                    && record_hash != ZERO_ID,
                Error::<T>::InvalidIdentifier
            );
            ensure!(
                !IdentityVersions::<T>::contains_key(subject_id, version),
            ...
            match IdentityHeads::<T>::get(subject_id) {
                ...
                Some(head) => {
                    ...
                    let prior = IdentityVersions::<T>::get(subject_id, head)
            ...
            Self::deposit_event(Event::IdentityVersionRecorded {
                subject_id,
                version,
                did_hash,
                fingerprint_hash,
        ...
        #[pallet::call_index(1)]
        #[pallet::weight(T::WeightInfo::record_credential_version())]
        pub fn record_credential_version(
            ...
            T::AdminOrigin::ensure_origin(origin)?;
                ...
                credential_id != ZERO_ID
                    && subject_id != ZERO_ID
                    && credential_type != ZERO_ID
                ...
                non_transferable,
                Error::<T>::TransferableIdentityCredentialForbidden
            );
            ensure!(
                !CredentialVersions::<T>::contains_key(credential_id, version),
                Error::<T>::VersionAlreadyExists
            );
            match CredentialHeads::<T>::get(credential_id) {
                ...
                Some(head) => {
                    ...
                    let prior = CredentialVersions::<T>::get(credential_id, head)
                        .ok_or(Error::<T>::InvalidVersion)?;
                    ensure!(
                        previous_record_hash == Some(prior.record_hash),
                        Error::<T>::PreviousHashMismatch
                    );
                    // A nontransferable credential may be revised or revoked,
                    // but another subject/type/issuer requires a new credential ID.
                    ensure!(
                        subject_id == prior.subject_id
                            && credential_type == prior.credential_type
                            && issuer_subject_id == prior.issuer_subject_id,
                        Error::<T>::CredentialBindingMismatch
                    );
                }
            }
            ...
            Self::deposit_event(Event::CredentialVersionRecorded {
                credential_id,
                subject_id,
                version,
                state,
