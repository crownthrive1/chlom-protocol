...
The build wrapper also sets `WASM_BUILD_WORKSPACE_HINT` to `substrate/chlom-l1` so an external `CARGO_TARGET_DIR` cannot detach Wasm dependency resolution from the committed native `Cargo.lock`. Runtime-only builds infer the same workspace in `build.rs`.
