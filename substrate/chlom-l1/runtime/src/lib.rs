// Adapted from the Polkadot SDK solochain template (Unlicense); see UPSTREAM-LICENSES.md.
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: alloc::borrow::Cow::Borrowed("chlom-runtime"),
    impl_name: alloc::borrow::Cow::Borrowed("chlom-runtime"),
    authoring_version: 1,
    // The version of the runtime specification. A full node will not attempt to use its native
    //   runtime in substitute for the on-chain Wasm runtime unless all of `spec_name`,
    //   `spec_version`, and `authoring_version` are the same between Wasm and native.
    // First CHLOM development runtime specification; not a public mainnet activation.
    spec_version: 1,
    impl_version: 1,
...
// Internal development fee denomination only. No economic token identity or price is assigned.
...
mod runtime {
    ...
    pub type ChlomUtility = pallet_chlom_utility;

    #[runtime::pallet_index(29)]
    pub type ChlomPolicy = pallet_chlom_policy;
}
